...
import { Phone, ArrowRight, RefreshCw, AlertCircle, CheckCircle2, Sparkles, UserPlus } from 'lucide-react';
...
export const OtpLoginForm: React.FC<OtpLoginFormProps> = ({ onSwitchToSignup }) => {
  ...
  const handleSendOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        if (res.code === 'ACCOUNT_NOT_FOUND' || res.message.toLowerCase().includes('no account')) {
          setErrorMessage('No account exists with this mobile number. Please sign up to continue.');
        } else {
  ...
  const handleVerifyOtp = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      if (res.success && res.user && res.token) {
        ...
        // Redirect to origin or target page
        setTimeout(() => {
      ...
      setErrorMessage('Verification failed. Please check the OTP code and try again.');
    } finally {
      setIsSubmitting(false);
    }
  };
    ...
    <div className="space-y-5">
      ...
      {errorMessage && (
        <div className="p-4 bg-rose-500/10 border border-rose-500/30 rounded-2xl text-rose-600 dark:text-rose-400 text-xs font-semibold space-y-2 animate-shake">
          ...
          {/* If Account Not Found, offer direct CTA to Switch to Signup */}
          {(errorCode === 'ACCOUNT_NOT_FOUND' || errorMessage.includes('No account exists')) && onSwitchToSignup && (
      ...
      {step === 'request' ? (
        ...
        <form onSubmit={handleSendOtp} className="space-y-4">
          <div className="space-y-2">
            ...
            <p className="text-[11px] text-muted-custom font-medium">An SMS with a 6-digit OTP code will be sent to your number.</p>
          </div>

          <button
            type="submit"
            disabled={isSubmitting || !validateMobile(mobileNumber)}
        ...
        <form onSubmit={handleVerifyOtp} className="space-y-4">
          <div className="bg-background-secondary p-4 rounded-2xl border border-border-custom/80 flex items-center justify-between">
            <div>
              <p className="text-[11px] text-muted-custom font-bold">OTP Code sent to:</p>
