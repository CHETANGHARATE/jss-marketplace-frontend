...
import { Mail, ArrowRight, RefreshCw, AlertCircle, CheckCircle2, Sparkles, UserPlus } from 'lucide-react';
...
export const EmailOtpLoginForm: React.FC<EmailOtpLoginFormProps> = ({ onSwitchToSignup }) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        if (res.code === 'ACCOUNT_NOT_FOUND' || res.message.toLowerCase().includes('no account')) {
          setErrorMessage('No account exists with this email address. Please sign up to continue.');
        } else {
  ...
  const handleVerifyOtp = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      if (res.success && res.user && res.token) {
        setSuccessMessage('Email OTP Verified! Logging in...');
      ...
      {step === 'request' ? (
        <form onSubmit={handleSendOtp} className="space-y-4">
          <div className="space-y-2">
            ...
            <p className="text-[11px] text-muted-custom font-medium">A 6-digit OTP verification code will be sent to your email.</p>
          </div>
