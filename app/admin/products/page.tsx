    ...
    <div className="space-y-8" onClick={() => setOpenMenuId(null)}>
      ...
      <div className="flex flex-col lg:flex-row gap-8 items-start">
        ...
        <div className="flex-1 bg-card border border-border/40 rounded-3xl p-6 sm:p-8 shadow-sm space-y-6 min-w-0 w-full">
          ...
          {/* Clean Pill-Style Horizontal Status Filter Tabs */}
          <div className="overflow-x-auto scrollbar-none pb-1">
            <div className="flex items-center gap-2 min-w-max p-1 bg-muted/20 border border-border/40 rounded-3xl">
              {[
                { id: 'all', label: 'All Products', count: extractProductsArray(allProductsData).length, color: 'bg-rose-500 text-white shadow-xs' },
                { id: 'pending', label: 'Pending Review', count: extractProductsArray(pendingProductsData).length, color: 'bg-amber-500 text-white shadow-xs', icon: Clock },
                { id: 'approved', label: 'Approved', count: extractProductsArray(allProductsData).filter(p => p.status === 'approved').length, color: 'bg-emerald-600 text-white shadow-xs', icon: CheckCircle2 },
                { id: 'draft', label: 'Draft', count: extractProductsArray(allProductsData).filter(p => p.status === 'draft').length, color: 'bg-slate-600 text-white shadow-xs' },
                { id: 'rejected', label: 'Rejected', count: extractProductsArray(allProductsData).filter(p => p.status === 'rejected').length, color: 'bg-rose-600 text-white shadow-xs', icon: XCircle },
                { id: 'hidden', label: 'Hidden', count: extractProductsArray(allProductsData).filter(p => p.status === 'hidden').length, color: 'bg-amber-600 text-white shadow-xs', icon: EyeOff },
                { id: 'archived', label: 'Archived', count: extractProductsArray(allProductsData).filter(p => p.status === 'archived').length, color: 'bg-purple-600 text-white shadow-xs', icon: Archive },
              ].map((tab) => {
